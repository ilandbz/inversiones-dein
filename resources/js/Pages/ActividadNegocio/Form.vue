...
const {
    agregarActividadNegocio, respuesta, errors, actualizarActividadNegocio
} = useActividadNegocio();
...
const crud = {
    'nuevo': async() => {
        await agregarActividadNegocio(form.value)
        form.value.errors = []
        if(errors.value) {
        ...
        if(respuesta.value.ok==1){
            form.value.errors = []
            hideModal('#modalactividad')
            Toast.fire({icon:'success', title:respuesta.value.mensaje})
            emit('onListar')
        }
    },
    'editar': async() => {
        await actualizarActividadNegocio(form.value)
        form.value.errors = []
        if(errors.value) {
            form.value.errors = errors.value
        }
...
<template>
    <teleport to="body">
        <form @submit.prevent="guardar">
            <div ref="modalEl" class="modal fade" id="modalactividad" tabindex="-1" aria-labelledby="modalactividadLabel" aria-hidden="true">
                <div class="modal-dialog modal-md">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="modalactividadLabel">{{ modalTitle }}</h1>
